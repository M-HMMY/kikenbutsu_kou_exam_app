 ...
 * **★ まだ 1 つも作っていません。**
 *
 * **甲種の「物理学及び化学」には計算が出ます。**
 * 乙 4 版では 6 種類（比熱と熱量 / 熱膨張 / 比重から質量 / 質量パーセント濃度 /
 * ヘスの法則 / 指定数量の倍数）を作りました。
 *
 * **甲種は「基礎的な」が外れる科目なので、これでは足りない可能性があります。**
 * 反応速度・化学平衡・電気化学まで要るかどうかは、
 * 出題範囲の洗い出し（`docs/syllabus.md`）で確かめてから決めてください。
 *
 * 上の `pick` / `fx` / `buildNumeric` が、値を振り直す仕組みです。そのまま使えます。
 */

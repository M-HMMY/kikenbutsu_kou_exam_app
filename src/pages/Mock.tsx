...
/**
 * 出題セット。本番は 45 問を 150 分で、**1 問あたり 3 分 20 秒**。
 *
 * **本番形式では、科目の比率を本番どおりにすること**（法令 15 / 物化 10 / 性消 20）。
 * 科目ごとに 6 割という基準がある以上、比率が違うと判定の意味がなくなる。
 * 短い形式は、比率を保ったまま縮める（7 問なら 3 / 2 / 2 のように）。
 *
 * **甲種に科目免除はない**（消防試験研究センターのよくある質問 Q16）。
 * 乙種版にあった「科目免除」の形式は、この版には入れていない。
 *
 * ただし `Config.fields` で科目を絞る仕組みは残してある。
 * **甲種は性消が 20 問**あるので、「性消だけ通しで解く」形式が要るなら、
 * `fields: ['property']` を持つプリセットを足せばよい。
 */
const PRESETS: (Config & { note: string })[] = [
  ...
  { label: 'ハーフ', count: 23, minutes: 76, note: '23 問 / 76 分。本番と同じペースで半分だけ' },
  { label: '短縮', count: 9, minutes: 30, note: '9 問 / 30 分。すきま時間に' },
];
  ...
  if (!session) {
    return (
      <div className="page">
        ...
        <p className="hint">
          本番は五肢択一式が {EXAM_QUESTIONS} 問、{EXAM_MINUTES} 分です（法令 15 問 / 物理学及び化学 10 問 / 性消 20 問）。
          <strong>合格には、3 科目それぞれで {Math.round(PASS_RATIO * 100)} % 以上が必要です。</strong>
          合計点ではありません。1 科目でも 6 割を切れば不合格になるので、この模試も科目ごとに判定します。
        </p>
        <p className="hint">
          <strong>甲種に科目免除はありません。</strong>
          乙種では、すでに免状を 1 つ持っていると他の類で法令と物化が免除されますが、
          <strong>甲種はどの科目も免除されません</strong>（→{' '}
        ...
        <p className="hint">
          ...
          この記述は 2026 年 9 月 15 日時点のものです。最新の試験要項は消防試験研究センターの公式サイトで確認してください。
        </p>
      </div>
    );
  }
        ...
        <section className="section">
          ...
          {narrowed !== undefined && (
            <p className="hint">
              ...
              <strong>本番はこの形式ではありません</strong>（甲種に科目免除はありません）。
            </p>
          )}

...
**甲種＝Class A** なので「A」で通じる。

**色は乙 4（朱 → 橙）と必ず分ける。**ホーム画面に並ぶので、
同系色だと見分けられない。甲種は全 6 類が対象なので、
乙 4 の「引火性液体だから火の色」という理屈は当てはまらない。
**最上位の資格**という意味で金を使っている。
...
# ---------------------------------------------------------------- 見た目の設定
# 背景は左上から右下へのグラデーション。甲種は最上位なので金を使った。
BG_FROM = (24, 32, 45)       # 黒鉄
BG_TO = (202, 138, 4)        # 金
FG = (255, 255, 255)         # 白い字
ACCENT = (125, 211, 252)     # 下線（金の背景から浮くよう水色）

RADIUS = 0.22                # 角の丸み（app.ico のみ。一辺を 1 としたときの半径）

# 「A」の字形。一辺を 1 とした座標で書く。
# 頂点から左右へ開く 2 本の斜めと、その間を渡す横棒の 3 本で組む。
TOP, BOTTOM = 0.235, 0.585        # 字の上端・下端
APEX_X = 0.500                    # 頂点の横位置
FOOT_HALF = 0.165                 # 下端での、中心から足までの距離
STROKE_HALF = 0.034               # 斜めの棒の太さの半分（横方向）
BAR_Y = (0.470, 0.525)            # 横棒の上下
...
def inside_glyph(x, y):
    """「A」の中かどうか。左右の斜め 2 本と、渡す横棒 1 本で組む。"""
    ...
    t = (y - TOP) / (BOTTOM - TOP)
    spread = t * FOOT_HALF
    if abs(x - (APEX_X - spread)) <= STROKE_HALF:
        return True
    if abs(x - (APEX_X + spread)) <= STROKE_HALF:
        return True

    # --- 横棒（2 本の斜めの間を、その高さでの幅に合わせて渡す） ---
    if BAR_Y[0] <= y <= BAR_Y[1] and abs(x - APEX_X) <= spread:
        return True
